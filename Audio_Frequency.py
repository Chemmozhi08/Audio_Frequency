import time
import numpy as np
import sounddevice as sd
import soundfile as sf
import librosa
import matplotlib.pyplot as plt
from sklearn.metrics.pairwise import cosine_similarity

# 1. Record Audio from User

def record_audio(filename, duration=5, sample_rate=22050):
    print(f"\nRecording '{filename}' for {duration} seconds")
    print("Please speak now...")

    audio = sd.rec(
        int(duration * sample_rate),
        samplerate=sample_rate,
        channels=1,
        dtype='float32'
    )

    for i in range(duration, 0, -1):
        print(f"Recording... {i} seconds remaining")
        time.sleep(1)

    sd.wait()
    sf.write(filename, audio, sample_rate)
    print(f"{filename} saved successfully.")

# Record two audio samples
record_audio("audio1.wav")
record_audio("audio2.wav")

# 2. Load Recorded Audio

audio1, sr1 = librosa.load("audio1.wav", sr=22050, mono=True)
audio2, sr2 = librosa.load("audio2.wav", sr=22050, mono=True)

# 3. Preprocessing
# Make both signals same length

min_length = min(len(audio1), len(audio2))
audio1 = audio1[:min_length]
audio2 = audio2[:min_length]

# Normalize amplitude

audio1 = audio1 / np.max(np.abs(audio1))
audio2 = audio2 / np.max(np.abs(audio2))

# 4. Frequency Domain Analysis (FFT)

fft_audio1 = np.abs(np.fft.fft(audio1))
fft_audio2 = np.abs(np.fft.fft(audio2))

# Keep only positive frequencies

half_len = len(fft_audio1) // 2
fft_audio1 = fft_audio1[:half_len]
fft_audio2 = fft_audio2[:half_len]

# 5. Similarity Calculation (Cosine Similarity)
# Normalize frequency vectors

fft_audio1_norm = fft_audio1 / np.linalg.norm(fft_audio1)
fft_audio2_norm = fft_audio2 / np.linalg.norm(fft_audio2)

similarity_score = cosine_similarity(
    fft_audio1_norm.reshape(1, -1),
    fft_audio2_norm.reshape(1, -1)
)[0][0]

# 6. Visualization (Single Frequency Graph)

plt.figure(figsize=(10, 5))
plt.plot(fft_audio1, label="Audio 1", alpha=0.75)
plt.plot(fft_audio2, label="Audio 2", alpha=0.75)

plt.text(
    0.5,
    0.9,
    f"Similarity Score = {similarity_score:.3f}",
    ha="center",
    fontsize=12,
    transform=plt.gca().transAxes,
    bbox=dict(facecolor="white", alpha=0.8)
)

plt.title("Frequency Domain Comparison of Two Audio Signals")
plt.xlabel("Frequency Bins")
plt.ylabel("Magnitude")
plt.legend()
plt.tight_layout()
plt.show()

# 7. Output Result

print("\nFinal Voice Similarity Score:", round(similarity_score, 3))
